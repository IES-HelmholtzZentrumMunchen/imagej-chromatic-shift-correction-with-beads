 ...
 */
public class MyGenericDialog extends Dialog implements ActionListener, TextListener,
    ...
    private int pathFieldCounter = 0;
    ...
    public MyGenericDialog(String title) {
    ...
    public MyGenericDialog(String title, Frame parent) {
...
//        super(title, parent);
    ...
    /** Adds an 8 column path field.
     * @param label			the label
     * @param defaultText		the text initially displayed
     */
    public void addPathField(String label, String defaultText) {
        addPathField(label, defaultText, 8, true, false, false);
    }
     ...
     * @param defaultText		text initially displayed
     * @param columns			width of the text field
     * @param folder Displays a select folder button
     * @param file Displays a select file button
     * @param save Displays a save dialog
     */
    public void addPathField(String label, String defaultText, int columns, boolean folder, boolean file, boolean save) {
        String label2 = label;
        if (label2.indexOf('_')!=-1)
            label2 = label2.replace('_', ' ');
        Label theLabel = makeLabel(label2);
        ...
        if (folder && file) {
            Button b1 = new Button("Select folder");

            if (save) {
                b1.setName("b:folder:save:" + pathFieldCounter);
            }
            else { // !save
                b1.setName("b:folder::" + pathFieldCounter);
            }
            ...
            if (save) {
                b2.setName("b:file:save:" + pathFieldCounter);
            }
            else { // !save
                b2.setName("b:file::" + pathFieldCounter);
            }
            ...
            if (save) {
                b.setName("b:file:save:" + pathFieldCounter);
            }
            else { // !save
                b.setName("b:file::" + pathFieldCounter);
            }
            ...
            if (save) {
                b.setName("b:folder:save:" + pathFieldCounter);
            }
            else { // !save
                b.setName("b:folder::" + pathFieldCounter);
            }
    ...
    public String getNextString() {
        ...
        if (recorderOn) {
            String s = theText;
            if (s!=null&&Character.isLetter(s.charAt(0))&&s.charAt(1)==':'&&s.charAt(2)=='\\')
...
//        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source==okay || source==cancel | source==no) {
            ...
            String[] tokens = s[s.length-1].split(":");
            ...
            if (tokens[0].equals("b")) {
                ...
                if (tokens[1].equals("file")) {
                    if (tokens[2].equals("save")) {
                        SaveDialog save = new SaveDialog("Save file", "Chromatic field", ".tif");
                        path = save.getDirectory() + save.getFileName();
                    }
                    else { // !tokens[2].equals("save")
                        path = IJ.getFilePath("Select file");
                    }
                }
                else { // tokens[1].equals("folder")
                    if (tokens[2].equals("save")) {
                        SaveDialog save = new SaveDialog("Save in folder", "Chromatic field", ".tif");
                        path = save.getDirectory();
                    }
                    else { // !tokens[2].equals("save")
                        path = IJ.getDirectory("Select folder");
                    }
                }

                ((Vector< TextField >)stringField).get(Integer.parseInt(tokens[3])).setText(path);
            }
            else { // !tokens[0].equals("b")
                notifyListeners(e);
            }
        }
    }
